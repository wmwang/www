...
if ($act == 'cat_rec')
{
    $rec_array = array(1 => 'best', 2 => 'new', 3 => 'hot', 4 => 'cat'); // zhangyh_100322  增加了一個元素 ，4 => 'cat'
    $rec_type = !empty($_REQUEST['rec_type']) ? intval($_REQUEST['rec_type']) : '1';
    $cat_id = !empty($_REQUEST['cid']) ? intval($_REQUEST['cid']) : '0';
	$topcat_id = !empty($_REQUEST['topcat_id']) ? intval($_REQUEST['topcat_id']) : '0'; // zhangyh_100322
    include_once('includes/cls_json.php');
    $json = new JSON;
    $result   = array('error' => 0, 'content' => '', 'type' => $rec_type, 'cat_id' => $cat_id,'topcat_id'=>$topcat_id); // zhangyh_100322

    $children = get_children($cat_id);
      //$smarty->assign($rec_array[$rec_type] . '_goods',      get_category_recommend_goods($rec_array[$rec_type], $children));    // 推薦商品zhangyh_100322
 ...
 /**
	...
	*/	
	if($rec_type==4){
		...
		$smarty->assign($rec_array[$rec_type] . '_goods',    assign_cat_goods($cat_id,$number,'123','goods'));    // 小類下的產品		
...
if (!$smarty->is_cached('index.dwt', $cache_id))
{
    ...
    $smarty->assign('promotion_info',  get_promotion_info()); // 增加一個動態顯示所有促銷信息的標籤欄

    $smarty->assign('invoice_list',    index_get_invoice_query());  // 發貨查詢
...
if ($limit >0)
{
$sql = 'SELECT gb.act_id AS group_buy_id, gb.goods_id, gb.ext_info, gb.goods_name, gb.act_desc, gb.end_time AS end_date , g.goods_thumb, g.goods_img, g.market_price, g.click_count '.
