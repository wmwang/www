...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
        ...
        $smarty->assign('bought_goods',        get_also_bought($goods_id));                      // 購買了該商品的用戶還購買了哪些商品
...
{
    $now = gmtime();
    $sql = "SELECT MAX(pg.goods_id), ga.act_id, ga.act_name, ga.act_desc, ga.goods_name, ga.start_time,
