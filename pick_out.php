...
if (empty($cat_id))
{
    ...
    $in_attr = $db->getCol($sql); //符合條件attr_id;
...
else
{
    ...
    $attr_picks = array(); //選擇過的attr_id

    /* 處理屬性,獲取滿足屬性的goods_id */
    if (!empty($_GET['attr']))
    {
        $attr_table = '';
        $attr_where = '';
    ...
    $in_attr = $db->GetCol($sql); // 符合條件attr_id;
    $in_attr = array_diff($in_attr, $attr_picks); // 除去已經選擇過的attr_id
