 ...
 * ECSHOP ΄ՂĚȝ
 * ============================================================================
 * ঈ蘋Ӑ 2005-2010 ɏ꣉¯ɍ胚쓐Ϟ髖⢱˹ӐȨ;ᣍ
 * ͸վ嘖糾http://www.ecshop.com㻍
 * ----------------------------------------------------------------------------
 * բ⻊ǒ븶הӉȭ쾣ᄺֻĜԚ⻓Oډ̒兒儵ć̡ς攳̐ⴺ&#171;���޸ĺ͍
 * ʹӃ㻲딊攳̐ⴺ&#171;ҔȎꎐΊ툎ꎄ﵄儔ٷⲼᣍ
 * ============================================================================
 * $Author: liuhui $
...
if (!$smarty->is_cached('article.dwt', $cache_id))
{
    ...
    $smarty->assign('article_categories',   article_categories_tree($article_id)); //΄Ղ疀ʷ
    $smarty->assign('categories',       get_categories_tree());  // 疀ʷ
    $smarty->assign('helps',            get_shop_help()); // ͸媰
    $smarty->assign('top_goods',        get_top10());    // ϺʛŅ
    $smarty->assign('best_goods',       get_recommend_goods('best'));       // ͆춉̆獊    $smarty->assign('new_goods',        get_recommend_goods('new'));        // ׮ЂɌƷ
    $smarty->assign('hot_goods',        get_recommend_goods('hot'));        // Ȉ壎ĕ&#141;
    $smarty->assign('promotion_goods',  get_promote_goods());    // ̘웉̆獊    $smarty->assign('related_goods',    article_related_goods($_REQUEST['id']));  // ̘웉̆獊	$smarty->assign('new_articles',    index_get_new_articles());   // ׮Ђ΄Ղ
    ...
    $smarty->assign('page_title',   $position['title']);    // ҳa᪌⍊    $smarty->assign('ur_here',      $position['ur_here']);  // 屇λփ
...
function get_article_info($article_id)
{
    ...
    if ($row !== false)
    {
        $row['comment_rank'] = ceil($row['comment_rank']);                              // Ӄ맆ۼ汰ȡջ
        $row['add_time']     = local_date($GLOBALS['_CFG']['date_format'], $row['add_time']); // ս̭쓊ἤϔʾ
