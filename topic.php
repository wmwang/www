...
if (!$smarty->is_cached($templates, $cache_id))
{
    ...
    $smarty->assign('ur_here',          $position['ur_here'] . '> ' . $topic['title']);     // 當前位置
    $smarty->assign('show_marketprice', $_CFG['show_marketprice']);
    $smarty->assign('sort_goods_arr',   $sort_goods_arr);          // 商品列表
    $smarty->assign('topic',            $topic);                   // 專題信息
    $smarty->assign('keywords',         $topic['keywords']);       // 專題信息
    $smarty->assign('description',      $topic['description']);    // 專題信息
    $smarty->assign('title_pic',        $topic['title_pic']);      // 分類標題圖片地址
    $smarty->assign('base_style',       '#' . $topic['base_style']);     // 基本風格樣式顏色
