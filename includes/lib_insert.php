...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
            ...
            $str.='<ul class="clearfix"><li class="goodsimg"><a href="'.$goods['url'].'" target="_blank"><img src="'.$goods['goods_thumb'].'" alt="'.$goods['goods_name'].'" class="B_blue" /></a></li><li><p class="name"><a href="'.$goods['url'].'" target="_blank" title="'.$goods['goods_name'].'">'.$goods['short_name'].'</a></p>'.'<font class="f1">'.$goods['shop_price'].'</font><br /></li></ul>';
        }
        $str .= '<div class="More"><a   onclick="clear_history()">' . $GLOBALS['_LANG']['clear_history'] . '</a></div>';
    }
    return $str;
}
